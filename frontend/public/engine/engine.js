...
document.writeln(
  "<script type='text/javascript' src='js/odontogramaGenerator.js'></script>"
);
document.writeln(
  "<script type='text/javascript' src='js/collisionHandler.js'></script>"
);
...
Engine.prototype.highlightMultiSelection = function (tooth) {
  "use strict";
  console.log("Resaltando la selección múltiple");
  try {
    ...
    console.log("Destacado del motor Selección múltiple: " + error.message);
  }
};
...
Engine.prototype.onTextBoxClicked = function (textBox) {
  ...
  // No hacer nada si ya hay texto (ya fue modificado)
  if (textBox.text && textBox.text.trim() !== "") return;

  // Prevenir doble ejecución
  if (this._textboxPrompting) return;
  this._textboxPrompting = true;

  var message = "Añadir código dental de 3 letras.";
  var text = prompt(message, "");

  this.setTextToTextBox(textBox, text);

  setTimeout(() => {
    this._textboxPrompting = false;
  }, 300);
};
...
Engine.prototype.mouseClickControls = function (event) {
  ...
  if (
    ...
    this.adultShowing = true;
    console.log("Configuración del odontograma en adultos");
    this.mouth = this.odontAdult;
    this.spaces = this.odontSpacesAdult;
    this.update();
  }
  ...
  if (
    ...
    console.log("Configuración del odontograma en niños");
...
Engine.prototype.setDamage = function (damage) {
  ...
  console.log("Daños en el ajuste del motor: " + damage);
  ...
  if (
    this.selectedDamage !== this.constants.DIASTEMA &&
...
Engine.prototype.loadPatientData = function (
  office,
  patient,
  number,
  treatmentNumber,
...
Engine.prototype.printPreview = function () {
  ...
  this.renderer.renderText("Especificaciones: ", 4, 1200, "#000000");
...
Engine.prototype.print = function () {
  ...
  windowContent += "<body >";
  windowContent +=
    '<p style="text-align: center;"><img src="' + dataUrl + '"></p>';
  windowContent += "</body>";
  windowContent += "</html>";
...
Engine.prototype.createMenu = function () {
  ...
  posX = posX + xSeparator;
  this.createMenuButton(posX, posY, buttonWidth, buttonHeight, "Protesis", 29);
  posX = posX + xSeparator;
  this.createMenuButton(posX, posY, buttonWidth, buttonHeight, "Tornillo", 30);
  ...
  this.createMenuButton(posX, posY, buttonWidth, buttonHeight, "Gastado", 37);
  posX = posX + xSeparator;
  this.createMenuButton(
    posX,
    posY,
